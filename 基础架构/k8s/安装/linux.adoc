...
nano /etc/bash_completion.d/ctr
....
+
....
_ctr()
{
    ...
    opts=$(ctr --help | awk '/^\s+[a-z]/ {print $1}' | sed 's/,$//')  # 移除逗号
    ...
    if [[ ${cword} == 1 ]]; then
        ...
        case $prev in
            images)
                COMPREPLY=( $(compgen -W "list pull remove import export tag" -- $cur) )
                ;;
            containers)
                COMPREPLY=( $(compgen -W "list create start stop delete" -- $cur) )
                ;;
            *)
                COMPREPLY=( $(compgen -W "${opts}" -- $cur) )
                ;;
